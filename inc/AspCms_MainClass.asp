...
Class TemplateClass
	...
	Public Function parseArr(Byval attr)
		...
		'die "获取可用标签参数"
		'dim d
		'for each d in strDictionary
		'	echo d & "=" & strDictionary(d) & "<br>"
		'next
		'echo "结束"& "<br>"
	End Function
	...
	sql ="select SortName,SortType,SortURL,sortID,(select count (*) from {prefix}Sort as a where a.ParentID=b.sortID) as subcount,SortFolder,SortFileName,GroupID,Exclusive,parentid,sortlevel from {prefix}Sort as b  where LanguageID="&setting.languageID&" and SortStatus=1 and ParentID="&pid&" order by SortOrder asc" 
...
'	end if
'nloopstr = replace(nloopstr,matchfield.value,left(replace(dropHtml(rsObj("PageDesc")),"{aspcms:page}",""),infolen))	
'else
'	infolen = parseArr(fieldArr)("len")
'	if isNul(infolen) then 
'	infolen = 200 
...
'	end if
'	'简述为空时，自动截取内容	
'nloopstr = replace(nloopstr,matchfield.value,left(replace(dropHtml(rsObj("content")),"{aspcms:page}",""),infolen))
'end if
							case "pic"
